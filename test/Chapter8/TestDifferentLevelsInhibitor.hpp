...
public:
    ...
    void RunDiseasedTissue(unsigned iT_pct) throw(Exception)
    {
        ...
        fill_dir_stream << "Project3/TestWithInitialRestriction/InhibitorLevel";
        fill_dir_stream << iT_pct << "pct/Seed";

        assert(iT_pct <= 100);
        double iT = 0.1e-9 * ((double) iT_pct)/100.0;
        PRINT_VARIABLE(iT);

        RunFillTissue(fill_dir_stream.str(),fill_length,100,iT);
        ...
        output_folder_stream << "Project3/DiseasedTissue/InhibitorLevel";
        ...
        // Add the cell height and age at death writers
